...
export class GitHubManager {
  ...
  async exchangeCode(code: string, signal?: AbortSignal) {
    ...
    await authStore.setGithub(auth);
    return auth;
  }

  async getSetupStatus(): Promise<SetupStatus> {
    const github = await authStore.getGithub();
    ...
    if (!github) missing.push("GitHub OAuth connection");
  ...
  async ensureRepository(
    ...
    const baseName = slugify(repositoryNameSuggestion || project.name, "codex-mobile-project");
  ...
  async commitFiles(params: {
    ...
    const removedPaths = params.previousPaths.filter((path) => !nextPaths.has(path));
    const treeEntries = [
  ...
  async enablePages(owner: string, repo: string, signal?: AbortSignal) {
    const github = await this.ensureAuthenticated(signal);
    const token = github.accessToken;
  ...
  async dispatchPagesWorkflow(owner: string, repo: string, branch: string, signal?: AbortSignal) {
    const github = await this.ensureAuthenticated(signal);
  ...
  async getLatestPagesRun(owner: string, repo: string, branch: string, signal?: AbortSignal) {
    const github = await this.ensureAuthenticated(signal);
  ...
  async getPages(owner: string, repo: string, signal?: AbortSignal) {
    const github = await this.ensureAuthenticated(signal);
