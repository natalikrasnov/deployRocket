...
class ProjectStore {
  ...
  async failProject(projectId: string, error: ProjectError) {
    const project = await this.updateProject(projectId, (current) => {
  ...
  async adoptRepository(
    ...
    await this.writePendingDefaultReadme(project, "Update pending generated project README");
    return clone(project);
  }

  async syncPendingDefaultReadme(projectId: string) {
    const project = await this.getProject(projectId);
    if (!project) return null;
    await this.writePendingDefaultReadme(project, "Update pending generated project README");
    return clone(project);
  ...
  private async writePendingDefaultReadme(project: Project, message: string) {
    if (!project.githubOwner || !project.githubRepo) return;
    if (project.lastCommittedPaths.length > 0) return;
    ...
    await githubManager.putTextFile({
      ...
      baseBranch: project.githubDefaultBranch ?? config.githubDefaultBranch,
      content: renderPendingDefaultReadme(project),
      message
    });
  }
...
function renderPendingDefaultReadme(project: Project) {
  return [
    "# " + project.name,
    "",
    project.summary,
    ...
    "## Generation Status",
    "",
    project.lastCommittedPaths.length
      ? "Application files have been generated and committed."
